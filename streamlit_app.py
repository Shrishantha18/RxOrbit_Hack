...
# Import your fine-tuned Donut model
from finetune_donut import DonutModel, load_model, extract_text_from_image
...
# Load Gemini API for analysis only (not OCR)
...
# Load your fine-tuned Donut model
@st.cache_resource
def load_donut_model():
    """Load the fine-tuned Donut model once"""
    try:
        model = load_model()  # Your function from finetune_donut.py
        return model
    except Exception as e:
        st.error(f"Failed to load Donut model: {str(e)}")
        return None

donut_model = load_donut_model()


# ---------------------------------------------------------
# OCR USING FINE-TUNED DONUT
# ---------------------------------------------------------
def donut_extract_text(uploaded_file):
    """Extract text using your fine-tuned Donut model"""
    try:
        if donut_model is None:
            return "OCR error: Donut model not loaded"
        
        # Read and prepare image
        img = uploaded_file.read()
        image = Image.open(io.BytesIO(img))
        
        # Use your Donut model's extraction function
        extracted_text = extract_text_from_image(donut_model, image)
...
def main():
    ...
    st.caption("Powered by Fine-tuned Donut OCR + Gemini Analysis")
    ...
    if uploaded:
        with st.spinner("🔍 Extracting text with Fine-tuned Donut..."):
            extracted_text = donut_extract_text(uploaded)
